...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    ...
    if (empty($username) || empty($password)) {
        ...
        $adminEmail = strtolower($username);
        if (!isGmailAddress($adminEmail)) {
            $error = 'Please use the admin Gmail account.';
        } else {
            $stmt = $conn->prepare("SELECT * FROM admin WHERE LOWER(username) = ? LIMIT 1");
        ...
        if (!isGmailAddress($email)) {
            $error = 'Please use your Gmail account, for example name@gmail.com.';
        } else {
        ...
        if (!$error && $user && ($user['password'] === $password || password_verify($password, $user['password']))) {
  ...
  <style>
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    .eye-btn {
      ...
      background:rgba(0,245,255,0.06); color:rgba(0,245,255,0.75);
    ...
    .forgot-panel p { font-size:12px; color:rgba(255,255,255,0.38); margin-bottom:12px; line-height:1.45; }
    .forgot-trigger {
      margin-top:12px;
      width:100%;
      border:1px solid rgba(0,245,255,0.14);
    ...
    .forgot-trigger:hover { border-color:rgba(0,245,255,0.35); background:rgba(0,245,255,0.07); }
...
<div class="login-wrapper">
  ...
  <div class="card" id="step2" style="display:none;">
    ...
    <form method="POST" action="login.php">
      ...
      <div class="field">
        ...
        <div class="password-wrap">
          ...
          <button type="button" class="eye-btn" data-eye-target="loginPassword" aria-label="Hold to show password">
    ...
    <button type="button" class="forgot-trigger" id="forgotTrigger" style="display:none;" onclick="toggleForgot()">
      <i class="fas fa-key"></i> Forgot password?
    </button>

    <div class="forgot-panel" id="forgotPanel">
      <p>Enter your registered Gmail account and the system will send a secure reset link automatically.</p>
      <form method="POST" action="forgot_password.php">
        ...
        <button type="submit" class="btn-login">
          <i class="fas fa-envelope-circle-check"></i> &nbsp;Send Reset Link
        </button>
      </form>
    </div>
...
function setupStep2(role) {
  ...
  document.getElementById('forgotTrigger').style.display = d.showRegister ? 'block' : 'none';
  if (!d.showRegister) document.getElementById('forgotPanel').style.display = 'none';
