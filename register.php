...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!isGmailAddress($email)) $errors[] = 'Please register with a Gmail account ending in @gmail.com.';
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("SELECT user_id FROM users WHERE LOWER(email) = ?");
    ...
    if (empty($errors)) {
        $studentValue = $student_id !== '' ? $student_id : null;
        $passwordHash = password_hash($password, PASSWORD_DEFAULT);
        $stmt = $conn->prepare("INSERT INTO users (student_id, full_name, grade, section, email, password) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $studentValue, $full_name, $grade, $section, $email, $passwordHash);
...
<head>
  ...
  <style>
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    .back-link {
      color:rgba(0,245,255,0.75); text-decoration:none; font-size:12px; font-weight:700;
      display:inline-flex; align-items:center; gap:7px; padding:8px 10px;
      border:1px solid rgba(0,245,255,0.14); border-radius:10px; background:rgba(0,245,255,0.04);
    }
    .back-link:hover { color:#00f5ff; border-color:rgba(0,245,255,0.35); }
    ...
    .eye-btn {
      ...
      background:rgba(0,245,255,0.06); color:rgba(0,245,255,0.75); cursor:pointer;
    }
    .eye-btn:hover { background:rgba(0,245,255,0.12); color:#00f5ff; }
    .password-rules { margin-top:7px; font-size:10px; color:rgba(255,255,255,0.35); line-height:1.45; }
...
<div class="wrapper">
  ...
  <div class="card">
    <div class="card-head">
      ...
      <a href="login.php?role=student" class="back-link"><i class="fas fa-arrow-left"></i> Back</a>
    </div>
    ...
    <form method="POST" action="register.php">
      <div class="grid2" style="gap:14px;">
        ...
        <div class="field span2">
          <label>Student ID <span style="color:rgba(255,255,255,0.2);font-weight:400;">(optional)</span></label>
        ...
        <div class="field span2">
          ...
          <input type="email" name="email"
                 ...
                 placeholder="yourname@gmail.com" pattern="^[^@\s]+@gmail\.com$" required>
        </div>
        ...
        <div class="field">
          ...
          <div class="password-wrap">
            ...
            <button type="button" class="eye-btn" data-eye-target="pwInput" aria-label="Hold to show password">
          ...
          <div class="password-rules">Use uppercase, lowercase, number, and symbol.</div>
        </div>
        ...
        <div class="field">
          ...
          <div class="password-wrap">
            <input type="password" name="confirm_password"
                   placeholder="Repeat password" required id="confirmPwInput">
            <button type="button" class="eye-btn" data-eye-target="confirmPwInput" aria-label="Hold to show password">
              <i class="fas fa-eye"></i>
            </button>
          </div>
        </div>
